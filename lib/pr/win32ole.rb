...
class WIN32OLE
  ...
  extend Windows::COM::Variant
  extend Windows::COM::Accessibility
  extend Windows::Unicode
  extend Windows::National
  ...
  def self.attach(dispatch)
    self.new.ole_set_member(dispatch)
  end
  ...
  def self.get_ptr_of_variant(pvar)
    ...
    when VT_UI1,VT_I2,VT_UI2,VT_I4,VT_UI4,VT_R4,VT_R8,VT_I8,VT_UI8,VT_INT,VT_UINT,VT_CY,VT_DATE,VT_ERROR,VT_BOOK,VT_ARRAY
  ...
  def self.val2variant_ptr(val,var,vt)
    ...
    if (vt & ~VT_BYREF) == VT_VARIANT
      ...
      if (vt & ~VT_BYREF) != var[0,2].unpack('S').first
        hr = VariantChangeTypeEx(var, var, @@lcid, 0, vt & ~VT_BYREF)
        if hr != S_OK
          raise RuntimeError, "failed to change type"
        end
      end
      ptr = get_ptr_of_variant(var)
    end
    ...
    for index in 0 ... count
      ...
      for iVar in 0 ... typeAttr[46,2].unpack('S').first
        ...
        if varDesc[32,4].unpack('L').first == VAR_CONST &&
          (varDesc[28,2].unpack('S').first & (VARFLAG_FHIDDEN | VARFLAG_FRESTRICTED | VARFLAG_FNONBROWSABLE)) == 0
        then
          len = 0.chr * 4
          bstr = 0.chr * 4
  ...
  def self.codepage=(cp)
    if code_page_installed(cp)
      ...
      case cp
      when CP_ACP, CP_OEMCP, CP_MACCP, CP_THREAD_ACP, CP_SYMBOL, CP_UTF7, CP_UTF8, 51932
        @@cp = cp
        if cp == 51932
  ...
  def ole_excepinfo2msg(execinfo)
    source = nil
    description = nil
    code,_,src,desc,_,_,_,fillin,scode = execinfo.unpack('SSLLLLLLL')
